...
class PatientArticleReranker:
    ...
    def _build_article_text(self, item: Dict) -> str:
    ...
    def rerank(self, patient_text: str, retrieved_articles: List[Dict], top_k: int) -> List[Dict]:
        ...
        for item in retrieved_articles:
            doc_text = self._build_article_text(item)
        ...
        reranked: List[Dict] = []
        for item, score in zip(retrieved_articles, scores):
            new_item = dict(item)
